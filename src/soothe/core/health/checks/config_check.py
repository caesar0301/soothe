...
def _check_env_vars_resolved(config: SootheConfig | None) -> CheckResult:
    ...
    unresolved = [
        f"{provider.name}.api_key" for provider in config.providers if provider.api_key and "${" in provider.api_key
    ]
...
def _check_soothe_home() -> CheckResult:
    ...
    if not home.exists():
        return CheckResult(
            ...
            details={"path": str(home), "remediation": "Run 'soothe config init' to create directory"},
        )
    ...
    required_subdirs = ["config", "runs", "logs"]
    ...
    if missing:
        return CheckResult(
            ...
            details={"path": str(home), "missing": missing, "remediation": "Run 'soothe config init'"},
        )
