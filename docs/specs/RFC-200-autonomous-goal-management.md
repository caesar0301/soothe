...
- **Goal DAG reflection**: Evaluate progress across multiple goals using Layer 2 JudgeResult
...
Layer 3's REFLECT stage receives Layer 2's JudgeResult and uses evidence_summary for goal DAG evaluation:
...
reflection = await planner.reflect(
    ...
    layer2_judgment=judge_result  # Layer 2 evaluation
)
# reflection includes:
...
while total_iterations < max_iterations and not GoalEngine.is_complete():
    ...
    +-- for each executing goal (PERFORM stage):
           |
           +-- Delegate to Layer 2's PLAN → ACT → JUDGE loop
           |      Receive: JudgeResult with evidence_summary, goal_progress
           |
           +-- REFLECT stage:
...
- **Evidence flow**: Layer 2 JudgeResult → Layer 3 REFLECT → goal directives → DAG restructuring
...
async def reflect(
    ...
    layer2_judgment: JudgeResult | None = None  # From Layer 2
) -> Reflection:
    """
    Returns:
        Reflection with:
